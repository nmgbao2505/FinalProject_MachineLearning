...
&emsp;Khi người dùng vừa truy cập vào website, họ sẽ được đưa ngay đến trang chủ. Xuất hiện ngay trước mắt người dùng sẽ là lời chào mừng đến từ người trợ lý ảo Pratt.

![Hình 4.1.1: Lời chào từ Pratt đến người dùng.](./figures/home/homepage_part_1.jpg)

&emsp;Tiếp sau đó là phần liệt kê và mô tả các chức năng mà Pratt có thể hỗ trợ người dùng. Đây là các chức năng đã được đề cập trong phần "Ý tưởng của ứng dụng" đã được đề cập ở trên. Và trong các phần tiếp sau đây, giao diện và cách thức hoạt động của từng chức năng sẽ được từng thành viên trong nhóm 8 trình bày chi tiết.

![Hình 4.1.2: Liệt kê và mô tả các chức năng.](./figures/home/homepage_part_2.jpg)

&emsp;Ngoài ra, để tăng thêm tính tương tác với người dùng, nhóm 8 có tạo thêm một biểu mẫu (form) để người dùng có thể trình bày các nguyện vọng hay đề xuất của mình đến nhóm. Trong tương lai, các thành viên trong nhóm có thể dựa theo yêu cầu của người dùng để cải thiện và bổ sung thêm các chức năng khác.
...
&emsp;Khi người dùng vừa truy cập đến chức năng này thì họ sẽ được yêu cầu đăng tải các file tài liệu của mình lên trang web. Hiện nay, trang web có hỗ trợ tra cứu trên bốn loại tài liệu phổ biến là các file: PDF (\*.pdf), Word (\*.docx), Text (\*.txt) và Markdown (\*.md). Vì mỗi loại tập tin sẽ có các phương pháp đọc dữ liệu đặc thù để phù hợp với mục tiêu đã đề ra của chức năng. Nên, tạm thời, nhóm 8 chỉ cho phép người dùng đăng tải một trong bốn loại file nêu trên.
...
&emsp;Sau khi người dùng đã hoàn tất bước "đăng tải tài liệu", họ có thể chuyển ngay đến giai đoạn quan trọng nhất là "Hỏi thông tin từ tài liệu (của chính bản thân mình)". Giao diện của bước này được truyền cảm hứng từ các ứng dụng Chatbot phổ biến hiện nay như ChatGPT, Gemini, Copilot, v.v..
...
&emsp;Chức năng này đã sử dụng mô hình "XLM-RoBERTa large" sau khi được fine-tune trên tập dữ liệu "Question Answering bằng tiếng Việt" để tra cứu tài liệu và trả về kết quả cho người dùng. Quy trình hoạt động của chức năng có thể được tóm tắt như sau:
...
![](./figures/code_generator/input1.png)

&emsp;Và đây là kết quả sau khi tạo code:

![](./figures/code_generator/output1.png)

&emsp;Kết quả khi chạy thử code trên trình biên dịch:

![](./figures/code_generator/result1.png)
...
![](./figures/code_generator/input2.png)

&emsp;Kết quả sau khi tạo code:

![](./figures/code_generator/output2.png)

&emsp;Kết quả khi chạy thử code trên trình biên dịch:

![](./figures/code_generator/result2.png)

&emsp;Sau khi hoàn thành, các file chứa những mã nguồn vừa được phát sinh sẽ được lưu trữ trong thư mục "codes".

![](./figures/code_generator/savefile.png)
...
![](./figures/code_generator/archi1.png)
...
&emsp;Cho một chủ đề để mô hình tạo sinh. Văn bản sẽ được tự động dịch sang tiếng Việt.
...
&emsp;Cho một đoạn văn hoặc một câu tiếng Anh sai ngữ pháp và mô hình sẽ tự động sửa lỗi.
...
Đối với chức năng "Sửa lỗi ngữ pháp":

- Đôi khi mô hình sẽ không thể sửa được hết tất cả các câu trong 1 đoạn văn bản  
  -> Giải pháp: Nên tách thành các đoạn nhỏ.
